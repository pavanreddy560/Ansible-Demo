   ...
   environment {     
    ...
    stages {
       ...
       stage('Ansible Deploy'){
          steps {    
             script {
                      sh """ sudo ansible-playbook -i myhosts ansible.yml --vault-password-file config.txt /root/etc/ansible/credentials.yml --extra-vars '{"ArtifactVersion": "${params.ArtifactVersion}", "nexus-ip": "${params.nexus_ip}"}' """
             }   
          } 
       }
    }      
